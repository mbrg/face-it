...
def intensityConspicuity(image):
    ...
    """
    # TODO make sure chaning im to image is fine
...
def gaborConspicuity(image, steps):
    ...
    for step in range(steps):
        ...
        numpy.add(gaborConspicuity, N(summedFeatures), out=gaborConspicuity, casting="unsafe")
...
def api(im, is_markMaxima=False):
    ...
    :param fileList: Text file containing input file names, one per line.
    :param is_intensityOutput: intensity conspicuity map
    :param is_gaborOutput: intensity conspicuity map
    :param is_rgOutput: rg conspicuity map
    :param is_byOutput: rg conspicuity map
    :param is_cOutput: color conspicuity map
    :param is_saliencyOutput: saliency map
    :param is_markMaxima: Mark maximum saliency in output image.
    :return: NamedTuple of images
    """

    logging.basicConfig(level=logging.DEBUG)
    SaliencyMaps = namedtuple('SaliencyMaps', ['intensity', 'gabor', 'rg', 'by', 'c', 'saliency'])

    # TODO COLOR_BGR2RGB COLOR_BGR2GRAY
    # im = cv2.imread(filename, cv2.COLOR_BGR2GRAY)  # assume BGR, convert to RGB---more intuitive code.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--intensityOutput', type=str, dest='intensityOutput', default='intensityOutput.jpg', action='store',
                        help="Filename for intensity conspicuity map,")
    parser.add_argument('--gaborOutput', type=str, dest='gaborOutput', default='gaborOutput.jpg', action='store',
                        help="Filename for intensity conspicuity map,")
    parser.add_argument('--rgOutput', type=str, dest='rgOutput', default='rgOutput.jpg', action='store',
                        help="Filename for rg conspicuity map,")
    parser.add_argument('--byOutput', type=str, dest='byOutput', default='byOutput.jpg', action='store',
                        help="Filename for by conspicuity map,")
    parser.add_argument('--cOutput', type=str, dest='cOutput', default='cOutput.jpg', action='store',
                        help="Filename for color conspicuity map,")
    parser.add_argument('--saliencyOutput', type=str, dest='saliencyOutput', default='saliencyOutput.jpg', action='store',
